  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/inventory/receive", {
        ...
        body: JSON.stringify({ productId: Number(productId), quantity: Number(quantity), location: location || undefined, reference: reference || undefined, notes: notes || undefined }),
      });
      if (!res.ok) throw new Error();
      toast({ message: "Stock received successfully", variant: "success" });
      router.push("/app/inventory");
    } catch {
      toast({ message: "Failed to receive stock", variant: "error" });
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
          <PackagePlus className="h-5 w-5 text-green-600" /> Receive Stock
        </h1>
      </div>
      ...
      <div className="rounded-xl border bg-white shadow-sm p-6 space-y-5">
        <div>
          ...
          {selected && (
            <p className="text-xs text-gray-500 mt-1">
              ...
              {selected.inventory?.location ? ` · Location: ${selected.inventory.location}` : ""}
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" placeholder="Warehouse A…" className={inputCls} value={location} onChange={(e) => setLocation(e.target.value)} />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium mb-1 text-gray-700">Reference</label>
          <input type="text" placeholder="PO-001, Supplier ref…" className={inputCls} value={reference} onChange={(e) => setReference(e.target.value)} />
        </div>
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button onClick={() => void handleSubmit()} disabled={saving} className="bg-green-600 hover:bg-green-700 text-white">
